...
When temporary storage is needed, use `$(jj workspace root)/.tmp`. If the current directory is not in a Jujutsu workspace, use a local `.tmp` directory instead of OS temporary directories.
