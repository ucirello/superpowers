...
For temporary files, use `$(jj workspace root)/.tmp`. If the current directory is not in a Jujutsu workspace, fall back to the local `.tmp` directory; do not use a global temporary directory.
