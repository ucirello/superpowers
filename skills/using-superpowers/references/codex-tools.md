...
BOOKMARKS=$(jj log -r '@ & bookmarks()' --no-graph -T 'bookmarks.map(|b| b.name()).join("\n") ++ "\n"')
```

- A task-specific workspace identified by the session is already isolated; do
  not create a nested workspace.
- Workspace path or name does not establish cleanup ownership. Use the owner
  recorded by the native harness or manual creation workflow, and leave the
  workspace in place when no ownership record exists.
- `BOOKMARKS` empty does not block local work. Create a bookmark only when a
  named publication reference is required.
...
workspace, record all work in Jujutsu and inform the user to use the App's
native controls:

- **"Create branch"** — the App's label for naming the publication branch,
  then pushing and creating a PR via App UI
...
The agent can still run tests, inspect Jujutsu changes, and output suggested
