...
async function jjInit(directory: string) {
  ...
  await execFileAsync('jj', ['init'], { cwd: directory });
}
```
...
**Trace chain:**
1. `jj init` runs in `process.cwd()` ← empty cwd parameter
...
- Layer 3: NODE_ENV guard refuses jj init outside workspace `.tmp`
