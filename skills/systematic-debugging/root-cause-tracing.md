...
**Fix:** Made `scratchDir` a getter that throws if accessed before `beforeEach`; it resolves to `$(jj workspace root)/.tmp` or the local `./.tmp` fallback when no Jujutsu repository is available.
