...
```typescript
const context = setupCoreTest(); // Returns { scratchDir: '' }
Project.create('name', context.scratchDir); // Accessed before beforeEach!
```
...
4. Test accessed `context.scratchDir` before beforeEach
5. setupCoreTest() returns `{ scratchDir: '' }` initially

**Root cause:** Top-level variable initialization accessing empty value

**Fix:** Made `scratchDir` a getter that resolves `.tmp` under `jj workspace root`, falls back to `.tmp` under the current directory when outside a Jujutsu workspace, and throws if accessed before `beforeEach`
