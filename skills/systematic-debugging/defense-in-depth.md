...
async function jjGitInit(directory: string) {
  ...
  if (process.env.NODE_ENV === 'test') {
    ...
    let scratchDir = normalize(resolve('.tmp'));
    try {
      const workspaceRoot = execFileSync('jj', ['workspace', 'root'], {
        encoding: 'utf8',
      }).trim();
      scratchDir = normalize(resolve(workspaceRoot, '.tmp'));
    } catch {
      // A local .tmp is the fallback when this is not a Jujutsu repository.
    }
...
- Layer 3: `WorkspaceManager` refuses `jj git init` outside `$(jj workspace root)/.tmp`, with local `./.tmp` fallback when no Jujutsu repository is available
