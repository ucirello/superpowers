...
async function jjGitInit(directory: string) {
  // In tests, refuse jj git init outside the repository-local temp directory.
  if (process.env.NODE_ENV === 'test') {
    const normalized = normalize(resolve(directory));
    const tmpDir = normalize(localTempRoot());
...
- Layer 3: `WorkspaceManager` refuses `jj git init` outside `$(jj workspace root)/.tmp` in tests, with local `.tmp` fallback outside Jujutsu
