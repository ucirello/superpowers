...
# Named bookmarks pointing at the working-copy commit (empty ⇒ no named bookmark)
BOOKMARKS_AT_WC=$(jj log -r @ -T 'bookmarks' --no-graph)
# See every workspace and its path (default vs secondary)
jj workspace list
```
...
| State | Menu | Cleanup |
|-------|------|---------|
| `WORKSPACE_PATH == DEFAULT_ROOT` (default workspace) | Standard 3 options | No workspace to clean up |
| `WORKSPACE_PATH != DEFAULT_ROOT`, named bookmark on `@` | Standard 3 options | Provenance-based (see Step 6) |
| `WORKSPACE_PATH != DEFAULT_ROOT`, no named bookmark on `@` | Reduced 2 options (no merge) | Externally managed — leave in place |

## Step 3: Determine Base Bookmark

The base bookmark is whatever this work forked from — usually named in the
plan, the conversation, or the feature bookmark's upstream/tracking remote.
If it is not already known, ask: "This bookmark split from <your best guess>
- is that correct?" Confirm before merging: merging into the wrong base is
expensive to undo.
...
**No named bookmark on `@` (externally managed workspace) — present exactly these 2 options:**

```
Implementation complete. You're on a working copy without a named bookmark (externally managed workspace).
...
# From a working copy without a named bookmark, create one then push:
...
```bash
jj bookmark delete <feature-bookmark>
# If the changes should also be abandoned (not merely un-bookmarked):
# jj abandon <change-revisions>
```
...
secondary workspace — and use the `WORKSPACE_PATH` / `DEFAULT_ROOT` /
`BOOKMARKS_AT_WC` values captured in Step 2, from before that directory
change.

**If `WORKSPACE_PATH == DEFAULT_ROOT`:** Default workspace, nothing extra to
clean up. Done.
...
```bash
# Forget by workspace name (from `jj workspace list` matching WORKSPACE_PATH)
jj workspace forget <workspace-name>
rm -rf "$WORKSPACE_PATH"
```

**Before removing the directory**, inspect for work that exists only there —
especially ignored files (jj does not track them) and a working-copy commit
not pointed at by any bookmark. Never `rm -rf` on your own initiative when
that inspection shows unique files. Show your human partner what is at
stake and ask:

```bash
jj -R "$WORKSPACE_PATH" st
# Also surface ignored/untracked files that would vanish with the directory:
# (e.g. find unmanaged files under WORKSPACE_PATH as appropriate for the repo)
```

```
Workspace removal blocked — these files may exist only in that workspace:
...
| 2. Create PR | - | yes | yes | - |
| 3. Keep as-is | - | - | yes | - |
| Discard (explicit request only) | - | - | - | yes (delete bookmark; abandon only if discarding the changes too) |
...
| "Removal blocked — force-deleting the directory is just finishing the cleanup" | The block means files may exist only in that workspace. Force-deleting destroys them permanently. Show your human partner and ask. |
