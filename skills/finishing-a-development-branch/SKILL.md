...
| State | Menu | Cleanup |
|-------|------|---------|
| Default workspace only (main checkout) | Standard 3 options | No secondary workspace to clean up |
| Secondary workspace under `.worktrees/` or `worktrees/`, with a named bookmark | Standard 3 options | Provenance-based (see Step 6) |
| Secondary / harness workspace, no feature bookmark | Reduced 2 options (no merge) | Externally managed — leave in place |
...
**No feature bookmark / externally managed workspace — present exactly these 2 options:**

```
Implementation complete. You're in an externally managed workspace (no feature bookmark on @).
...
```bash
# Get default workspace root for CWD safety (parent of .worktrees/ path, or
# the sole workspace root when already on the main checkout)
MAIN_ROOT=$(jj workspace root)
# If currently in a secondary workspace under .worktrees/, MAIN_ROOT is that
# workspace; discover the default workspace path via jj workspace list and cd there.
cd "$MAIN_ROOT"

# Fetch latest remote bookmarks
jj git fetch

# Merge first — verify success before removing anything.
# Create a merge commit with both base and feature as parents, on top of base:
jj new <base-bookmark> <feature-bookmark>
# Move the base bookmark to the merge result
...
```bash
# Ensure a bookmark points at the work to push
jj bookmark create <feature-bookmark> -r @   # if not already named
jj git push --bookmark <feature-bookmark>
# From a workspace with no bookmark yet, create one then push:
...
tooling — its CLI if one is available (`gh` is fine), or the creation URL most forges
...
```bash
jj bookmark delete <feature-bookmark>
# If the bookmark must be removed even when not fully merged, still:
# jj bookmark delete <feature-bookmark>
```
...
main repo root — workspace removal must run from outside the secondary
workspace — and use the `WORKSPACE_PATH` value captured in Step 2, from
...
**If the working copy still has unsettled changes** before forget: the
workspace holds files that exist nowhere else — undescribed plans, notes,
or scratch work. Never destroy them on your own initiative. Show your human
partner what is at stake and ask:

```bash
jj -R "$WORKSPACE_PATH" st
# or: (cd "$WORKSPACE_PATH" && jj st)
```

```
Workspace cleanup refused — these changes were never finished:
...
For option 1, finish the change with jj. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local syntax from project instructions and git log ALWAYS wins when it differs from Go guidance.
