...
If composing a change description, repository-local instructions and the repository-prescribed `git log` syntax take precedence. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Apply compatible Go guidance for clarity and useful rationale without imposing a fixed message, prefix, type, scope, subject, body, template, or example.
...
3. **Real file paths** - `$(jj workspace root)/.tmp/payment-system` in a Jujutsu workspace, or `.tmp/payment-system` when Jujutsu is unavailable, not "a project"
