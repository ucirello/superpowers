...
- Deleting ignored scratch (e.g. a clean of untracked files) will destroy
  the workspace; if that happens, recover from `jj log` / the op log.
...
feature work started from, e.g. `jj log -r 'heads(::@ & ::main)' -T 'commit_id' --no-graph`) and include the
printed path in the final review dispatch, so the final reviewer reads
one file instead of re-deriving the change-range diff with jj commands. Dispatch
...
When the final whole-change review is clean and its fixes are merged,
delete this plan's workspace (`rm -rf <workspace>`) — the repository history is
