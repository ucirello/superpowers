...
**Note:** Pass the project root as `--project-dir` so mockups persist in `.rocketclaw/brainstorm/` and survive server restarts. Without it, files go under `.tmp/brainstorm-*` (relative to the project/cwd) and get cleaned up on stop. Remind the user to add `.tmp/` and `.rocketclaw/` to `.gitignore` if they're not already there.
...
If the session used `--project-dir`, mockup files persist in `.rocketclaw/brainstorm/` for later reference. Only `.tmp/brainstorm-*` ephemeral sessions get deleted on stop.
