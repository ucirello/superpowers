...
**Note:** Pass the project root as `--project-dir` so mockups persist in `.rocketclaw/brainstorm/` and survive server restarts. Without it, files go to `$(jj workspace root)/.tmp/brainstorm/`, or the current directory's `.tmp/brainstorm/` outside a Jujutsu repository, and get cleaned up. Remind the user to add `.rocketclaw/` and `.tmp/` to the repository's ignore rules if they are not already there.
...
# (on Windows, call a Bash executable from the PowerShell tool).
...
If the session used `--project-dir`, mockup files persist in `.rocketclaw/brainstorm/` for later reference. Only repo-local or fallback `.tmp/brainstorm/` sessions get deleted on stop.
