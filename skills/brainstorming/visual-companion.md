...
**Note:** Pass the project root as `--project-dir` so mockups persist in `.rocketclaw/brainstorm/` and survive server restarts. Without it, files go under the workspace `.tmp/` and get cleaned up. Remind the user to add `.rocketclaw/` to `.gitignore` if it's not already there.
...
If the session used `--project-dir`, mockup files persist in `.rocketclaw/brainstorm/` for later reference. Only workspace `.tmp/` sessions get deleted on stop.
