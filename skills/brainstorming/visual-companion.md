...
**Note:** Pass the project root as `--project-dir` so mockups persist in `.rocketclaw/brainstorm/` and survive server restarts. Without it, files go to `$(jj workspace root)/.tmp` and get cleaned up; outside a Jujutsu repository, the launcher falls back to `.tmp` under the current directory. Remind the user to add `.rocketclaw/` to the repository's ignore rules if it is not already there.
...
If the session used `--project-dir`, mockup files persist in `.rocketclaw/brainstorm/` for later reference. Only generated `.tmp/brainstorm-*` sessions get deleted on stop.
