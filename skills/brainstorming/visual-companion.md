...
**Note:** Pass the project root as `--project-dir` so mockups persist in `.rocketclaw/brainstorm/` and survive server restarts. Without it, files go under the workspace `.tmp/brainstorm-…` (or local `.tmp/` if no jj workspace) and get cleaned up on stop. Remind the user to add `.rocketclaw/` and `.tmp/` to `.gitignore` if they're not already there.
