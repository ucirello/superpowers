...
**Note:** Pass the project root as `--project-dir` so mockups persist in `.rocketclaw/brainstorm/` and survive server restarts. Without it, files go to `$(jj workspace root)/.tmp/brainstorm/`, with local `.tmp/brainstorm/` as the fallback when Jujutsu is unavailable or the directory is not a Jujutsu workspace, and get cleaned up. Remind the user to add `.rocketclaw/` and `.tmp/` to the repository's ignore rules if they are not already there.
...
If the session used `--project-dir`, mockup files persist in `.rocketclaw/brainstorm/` for later reference. Sessions under the workspace-local `.tmp/brainstorm/` directory get deleted on stop.
