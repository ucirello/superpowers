...
**Note:** Pass the project root as `--project-dir` so mockups persist in `.rocketclaw/brainstorm/` and survive server restarts. Without it, files go to `$(jj workspace root)/.tmp/rocketclaw/brainstorm/`, or the current directory's `.tmp/rocketclaw/brainstorm/` when not in a Jujutsu workspace, and get cleaned up. Remind the user to add `.rocketclaw/` and `.tmp/` to `.gitignore` if they are not already there.
...
If the session used `--project-dir`, mockup files persist in `.rocketclaw/brainstorm/` for later reference. Only sessions under the workspace-local `.tmp/rocketclaw/brainstorm/` namespace get deleted on stop.
