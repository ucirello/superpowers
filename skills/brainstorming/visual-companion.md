...
#           "screen_dir":"/path/to/project/.context/brainstorm/12345-1706000000/content",
#           "state_dir":"/path/to/project/.context/brainstorm/12345-1706000000/state"}
```
...
**Finding connection info:** The server writes its startup JSON to `$STATE_DIR/server-info`. If you launched the server in the background and didn't capture stdout, read that file to get the URL and port. When using `--project-dir`, check `<project>/.context/brainstorm/` for the session directory.

**Note:** Pass the project root as `--project-dir` so mockups persist in `.rocketclaw/brainstorm/` and survive server restarts. Without it, files go under `$(jj workspace root)/.tmp/brainstorm-*`, falling back to the current directory's `.tmp/brainstorm-*` outside a Jujutsu repository, and get cleaned up. Remind the user to add `.rocketclaw/brainstorm/` to the repository's ignore rules if it isn't already there.
...
If the session used `--project-dir`, mockup files persist in `.rocketclaw/brainstorm/` for later reference. Sessions under `.tmp/brainstorm-*` are marked ephemeral and deleted on stop.
