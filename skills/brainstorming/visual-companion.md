...
**Note:** Pass the project root as `--project-dir` so mockups persist in `.rocketclaw/brainstorm/` and survive server restarts. Without it, files go to `$(jj workspace root)/.tmp` and get cleaned up; outside a Jujutsu workspace, the launcher falls back to `.tmp` under the current directory. Remind the user to add `.rocketclaw/` to `.gitignore` if it's not already there.
