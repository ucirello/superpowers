...
**Note:** Pass the project root as `--project-dir` so mockups persist in `.rocketclaw/brainstorm/` and survive server restarts. Without it, files go to `$(jj workspace root)/.tmp/rocketclaw`, or the current directory's `.tmp/rocketclaw` outside a Jujutsu repository, and get cleaned up. Remind the user to ignore `.rocketclaw/` according to the repository's local conventions if it is not already ignored.
...
If the session used `--project-dir`, mockup files persist in `.rocketclaw/brainstorm/` for later reference. Sessions under a workspace-local `.tmp` get deleted on stop.
