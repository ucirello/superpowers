...
**Note:** Pass the project root as `--project-dir` so mockups persist in `.rocketclaw/brainstorm/` and survive server restarts. Without it, files go to `$(jj workspace root)/.tmp/rocketclaw/`, or local `.tmp/rocketclaw/` outside a jj repository, and get cleaned up. Remind the user to ignore `.rocketclaw/` with the repository's ignore rules if needed.
...
If the session used `--project-dir`, mockup files persist in `.rocketclaw/brainstorm/` for later reference. Sessions under `$(jj workspace root)/.tmp/rocketclaw/`, or local `.tmp/rocketclaw/` outside a jj repository, get deleted on stop.
