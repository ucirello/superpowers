...
#           "screen_dir":"/path/to/project/.rocketclaw/brainstorm/<session-id>/content",
#           "state_dir":"/path/to/project/.rocketclaw/brainstorm/<session-id>/state"}
```
...
**Note:** Pass the project root as `--project-dir` so mockups persist in `.rocketclaw/brainstorm/` and survive server restarts. Without it, files go to `$(jj workspace root)/.tmp/rocketclaw/brainstorm/`, or the current directory's `.tmp/rocketclaw/brainstorm/` outside a Jujutsu workspace, and get cleaned up. Remind the user to add `.rocketclaw/` to the repository's ignore file if it is not already there.
...
# server survives across turns. Keep --foreground so the agent environment, not the
# script, owns backgrounding. The launcher is a .sh, so invoke it via bash
# (on Windows, call a compatible bash.exe from the PowerShell tool).
...
If the session used `--project-dir`, mockup files persist in `.rocketclaw/brainstorm/` for later reference. Only sessions under a workspace-local `.tmp/rocketclaw/brainstorm/` directory get deleted on stop.
