...
#
# Kills the server process. Only deletes ephemeral .tmp/brainstorm-*
# sessions. Persistent directories (.rocketclaw/) are kept so mockups
...
if [[ -f "$PID_FILE" ]]; then
  ...
  # Only delete the launcher's workspace-local ephemeral session directories.
  case "$SESSION_DIR" in
    */.tmp/brainstorm-*) rm -rf "$SESSION_DIR" ;;
  esac
