...
#
# Kills the server process. Only deletes generated .tmp session directories.
...
if [[ -f "$PID_FILE" ]]; then
  ...
  # Only delete generated ephemeral workspace-local directories.
  if [[ "$SESSION_DIR" == */.tmp/brainstorm-* || "$SESSION_DIR" == */.tmp/brainstorm ]]; then
    rm -rf "$SESSION_DIR"
  fi
