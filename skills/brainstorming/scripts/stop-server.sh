...
#
# Kills the server process. Only deletes session directories in the local
# .tmp/rocketclaw/brainstorm namespace. Persistent directories (.rocketclaw/)
...
if [[ -f "$PID_FILE" ]]; then
  ...
  # Only delete sessions from the dedicated repository-local temp namespace.
  if [[ "$SESSION_DIR" == */.tmp/rocketclaw/brainstorm/* ]]; then
    rm -rf "$SESSION_DIR"
  fi
