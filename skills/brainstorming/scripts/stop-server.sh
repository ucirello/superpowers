...
# workspace-local .tmp/rocketclaw/. Persistent directories (.rocketclaw/)
...
if [[ -f "$PID_FILE" ]]; then
  ...
  # Only delete workspace-local scratch sessions.
  if [[ "$SESSION_DIR" == */.tmp/rocketclaw/brainstorm-* || "$SESSION_DIR" == .tmp/rocketclaw/brainstorm-* ]]; then
    rm -rf "$SESSION_DIR"
  fi
