...
function localTempRoot() {
  try {
    const root = childProcess.execFileSync('jj', ['workspace', 'root'], {
      encoding: 'utf-8',
      stdio: ['ignore', 'pipe', 'ignore']
    }).trim();
    if (root) return path.join(root, '.tmp');
  } catch (e) { /* use a local fallback outside a Jujutsu repository */ }
...
function readRocketClawVersion() {
  ...
  for (const manifest of manifests) {
    try {
      ...
      // Packaged Codex plugins may omit package.json; try the next manifest.
    }
  }
...
function brandMarkup() {
  ...
  return '<div class="brand"><span class="brand-copy">RocketClaw v' + version + '</span></div>';
}
