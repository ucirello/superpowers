...
# Some Windows Bash environments reap nohup background processes. Auto-foreground when detected.
...
# Resolve the agent environment PID (grandparent of this script).
# $PPID is the ephemeral shell the environment spawned to run us — it dies
# when this script exits. The environment itself is $PPID's parent.
